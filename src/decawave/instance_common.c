...
int instance_run(void)
{
    ...
    if(done == INST_DONE_WAIT_FOR_NEXT_EVENT_TO) //we are in RX and need to timeout (Tag needs to send another poll if no Rx frame)
    {
        ...
        {
            instance_data[instance].instancetimer = deca_millis() + instance_data[instance].tagSleepTime_ms; //set timeout time
            instance_data[instance].instancetimer_en = 1; //start timer
        }
        if(instance_data[instance].mode == TAG_TDOA)
        {
            instance_data[instance].instancetimer = deca_millis() + instance_data[instance].tagBlinkSleepTime_ms; //set timeout time
            instance_data[instance].instancetimer_en = 1; //start timer
        }
    ...
    {
        if(instance_data[instance].instancetimer < deca_millis())
        {
			event_data_t dw_event;
            instance_data[instance].instancetimer_en = 0;
			dw_event.rxLength = 0;
